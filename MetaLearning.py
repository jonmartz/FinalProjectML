...
def get_feature_importances():
    ...
    pred = meta_model.get_booster().predict(xgb.DMatrix(X), pred_contribs=True)
    shap_values = pred.mean(axis=0)
    rows = []
    for i in range(len(features)):
        feat_id = 'f%d' % i
        if feat_id in weights:
